...
En réalité, nous ne pouvons manipuler le texte sous *WORD* que dans les limites des possibilités voulues par son propriétaire, en l'occurrence [Microsoft](https://www.microsoft.com/en-us/). Prenons un exmple. Je peux souligner un mot ou une suite de mots ; je peux également les mettre en italique ou en gras ; je peux même mettre une lettre en exposant ou en indice ; je peux encore rechercher certaines lettres, mots ou suites de mots dans un texte *WORD* (essayer avec la combinaison de touches CRTL + F), puis les remplacer par d'autres lettres, mots ou suites de mots (essayer avec la combinaison de touches CTRL + R, après avoir exécuté l'instruction correspondant à la touche CRTL + F) ; je peux faire un nombre significatif d'autres tâches de manipulation de textes sous *WORD*. Cependant, je ne peux pas effectuer des tâches non programmées dans la suite logicielle *MS WORD*. Par exemple, je peux vouloir rechercher des suites de lettres (un mot étant essentiellement une suite de lettres, ou plus exactement une suite de **caractères**) tout en laissant la possibilité à plus d'une option de sélection de caractères, comme suit :
*la lettre « T » en majuscule ou en minuscule suivie soit de la lettre « e », suivie soit de la lettre « x » ou de la lettre « s », suivie de la lettre « t », suivie ou non de la lettre « e », suivie ou non de la lettre « s »*. 

Mais pourquoi voudrais-je effectuer une recherche aussi débile 😡 !?

Pensez-y un instant ! La recherche (débile ☺️) que nous venons d'effectuer nous permet de rechercher simultanément les mots « texte » et « test » avec un « t » majuscule ou minuscule au début, aussi bien au singulier qu'au pluriel, soit huit formes différentes. Maintenant, pensez à une situation dans laquelle, ayant confié la saisie de votre mémoire de Master 2 à votre petit-frère informaticien car il a une plus grande aptitude à la bureautique informatique que vous, vous vous rendez compte qu'il a tendance à confondre les deux mots (« texte » et « test »), au singulier comme au pluriel, en début ou en milieu de phrase. Vous menez une recherche sur l'analyse des textes littétaires et le mot « texte » survient plusieus centaines d'une de fois dans votre mémoire. Le mot « test » survient aussi dans votre mémoire mais beaucoup moins fréquemment, car vous avez procédé à des tests statistiques et lexicométriques pour les besoins de votre analyse. Vous avez tout juste encore quelques jours pour déposer votre mémoire. Votre encadreur attend le mémoire ce même jour en soirée pour une relecture finale. Vous êtes épuisés et désespérés. Vous n'avez tout simplement pas le temps de relire tout le mémoire et d'identifier avec exactitude toutes les fois où votre petit frère a confondu les mots « texte » et « test ». Le pire, c'est que ces mots apparaissent et sont confondus aussi bien au singulier qu'au pluriel, en début comme au milieu de phrase. 

Eh bien ! Si votre mémoire existe uniquement au format *WORD*, il n'y a malheureusement pas grand chose que vous puissiez y faire. Certes en utilisant les combinaisons de touces *CRTL + F* et *CRTL + R* et en entrant à le mot « test », *WORD* vous renvoie toutes les occurrences de ce mot. Le problème, c'est que vous devez encore parcourir le texte intégralement, vérifier occurrence après occurrence, puis identifier chaque fois qu'une occurrence devrait ou non être corrigée. Une fois que vous en avez terminé avec les occurrences de « test », il vous faut ensuite rechercher les occurrences de « texte » pour réaliser les mêmes vérifications et éventuellement, les corrections nécessaires. Cet exemple n'est qu'un cas mineur qui illustre les limites d'un format texte propriétaire tel que *MS WORD*.

Beaucoup moins connus des utilisateurs d'applications d'édition de textes, les formats *.txt (texte pure)* et *.xml (Extensible Markup Language)* sont dits « non propriétaires » et « libres » ou  encore « open source ». Ceci signifie que le format de texte ne limite pas la portée des manipulations qui peuvent y être apportées. Ensuite, cela signifie que le texte peut s'échanger d'un système d'exploitation à un autre, ou d'une plateforme à une autre, sans risquer de subir des modifications dues à des restrictions d'accès en lecture ou en écriture. Peut-être avez-vous déjà fait l'expérience déprimante d'un texte devenu illisible en le transférant de votre téléphone Android à votre ordinateur équipé du système d'exploitation *Microsoft* et du logiciel *MS WORD* ? Cette expérience est aussi courante lorsque l'on transfère les fichiers *WORD* vers un ordinateur équipé du système d'exploitation *MacOS (Macintosh Operating System)* de la société [Apple Inc](https://www.apple.com/). Par exemple, en imaginant la situation précédente dans laquelle votre mémoire se retrouve avec des erreurs d'orthographe dues à la confusion de deux mots proches par leur graphie et leur sonorité (à l'oral), il est plus facile d'y remédier dans un texte au format non propriétaire tel que *.txt* ou encore *.xml*. Cette remédiation peut se faire à travers le recours à un langage de programmation tel que [Python](https://www.python.org/), ou même par l'application d'[expressions régulières](https://fr.wikipedia.org/wiki/Expression_r%C3%A9guli%C3%A8re) dans une simple [interface en ligne de commande](https://fr.wikipedia.org/wiki/Interface_en_ligne_de_commande).

Assez de formats de texte 😫 😫 😫 ! venons-en à notre belle lecture distante.

Pour notre entraînement à la lecture distante, nous irons chercher justement un texte dans la bibliothèque numérique du [Projet Gutenberg](https://www.gutenberg.org/). J'ai une fascination personnelle (je ne sais trop pourquoi) pour *Les Fleurs du mal* de Charles Baudelaire, publié pour la première fois en 1857. Il s'agit donc d'une oeuvre d'un auteur ancien dont l'accès n'est plus soumis aux droits d'auteur. Si vous êtes intéressés par la même oeuvre, vous pouvez directement accéder à sa version texte pure (*.txt*) en cliquant [ici](https://www.gutenberg.org/cache/epub/6099/pg6099.txt). 
...
4. Commencez la sélection à partir de ce poème (y compris son titre). Maintenez la sélection puis déroulez progressivement la page du haut vers le bas, jusqu'au dernier poème intitulé « LES METAMORPHOSES DU VAMPIRE ». Arrêtez la sélection à la fin de ce dernier poème. 
5. Effectuez la copie (vous pouvez utiliser la combinaison de touches *CTRL + C*), puis basculez de nouveau vers la page d'accueil de la plateforme *Voyant Tools*. 
6. Collez le texte copié dans le champ de texte qui apparaît en encadré au milieu de la page (vous pouvez utiliser la combinaison de touches *CTRL + V*).
7. Cliquez sur le bouton **Révéler** en bleu, à droite du champ de texte.

Alternativement, vous pouvez choisir d'effectuer les tâches à partir de l'étape numéro 3 jusqu'à l'étape numéro 6 en un seul bloc. Pour se faire, copiez l'URL (le lien) de la page web de l'oeuvre *Le Fleurs du mal* que voici: https://www.gutenberg.org/cache/epub/6099/pg6099.txt, puis collez-le dans le champ de texte de la plateforme *Voyant Tools*.
**Attention !!!** En choisissant cette méthode simplifiée, le texte que *Voyant Tools* traitera comportera des extras correspondant aux métadonnées de la page web, y compris la préface de l'oeuvre, ainsi que d'autres informations non littéraires. Pour éviter ces intrusions qui peuvent considérablement fausser l'analyse, il est recommandé de s'en tenir aux sept étapes indiquées plus haut.

En outre, *Voyant Tools* vous offre, à partir du champ de texte, la possibilité d'insérer un ou plusiers textes, un ou plusieurs corpus déjà constitués, depuis votre ordinateur. Par exemple, vous pouvez choisir de sauvegarder une copie de l'oeuvre *Les Fleurs du mal* dans votre ordinateur, pour une utilisation ultérieure (**Attention !! *Voyant Tools* ne sauvegarde pas les textes que vous y avez copiés ou insérés depuis votre ordinateur**). Si vous souhaitez suvegarder le texte de l'oeuvre dans votre ordinateur, procédez comme suit :
- mettez-vous sur le bureau de votre ordinateur ; dans le champ de recherche sur l'angle gauche en bas de votre écran, à côté du logo de *Windows*, entrez l'expression (la chaîne de caractères) « blocnotes » (il s'agit du nom d'un éditeur de texte libre), puis cliquez sur « Entrez » ; 
- ouvrez l'éditeur de texte *blocnotes*, puis collez le texte de l'oeuvre *Le Fleurs du mal* ;
- sauvegardez le fichier dans un répertoire de votre ordinateur, en prenant le soin de lui donner un nom suivi de l'extension *.txt*.

La prochaine fois que vous voudrez analyser ce texte sous *Voyant Tools*, vous aurez le choix entre copier et coller le texte à partir du fichier sauvegardé, ou importer le fichier dans *Voyant Tools* directement de votre ordinateur.

**N.B. Les différentes procédures décrites ici n'ont été testées que sur un ordinateur. Il n'est pas certain que vous puissiez les reproduire comme telles à partir d'un téléphone portable !!**

Si tout s'est passé comme prévu, après l'étape 7, la plateforme *Voyant Tools* devrait maintenant affichier des graphiques coloriés et chatoyants, qui n'attendent plus que votre perspicacité pour être interprétés à bon escient. 

BRAVO !! VOUS AVEZ RÉALISÉ VOTRE PREMIÈRE LECTURE DISTANTE 👏 👏 👏